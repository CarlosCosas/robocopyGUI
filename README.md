...
- **Mirror Mode** - Synchronize folders with `/MIR` flag (mirrors source to destination)
...
- **Long Path Support** - Handle Windows paths exceeding 260 characters
...
- **Smart Exclusions** - Automatically excludes junction points and desktop.ini files
...
- **robocopy.ps1** - Core engine that orchestrates Robocopy operations
  ...
  - Handles long paths (>260 characters) automatically
...
- `/XF desktop.ini` - Exclude desktop.ini files
- `/MT:n` - Multithreading (configurable)
...
1. **Test First** - Always use `-Validate` or `-DryRun` before running actual mirror operations
